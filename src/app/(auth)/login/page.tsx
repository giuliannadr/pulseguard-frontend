    ...
    <div className="min-h-dvh flex items-center justify-center p-6 relative">
      ...
      <div className="w-full max-w-[380px] fade-up relative z-10">
        ...
        <div className="mb-10 text-center">
          ...
          <p className="text-[var(--text-muted)] text-sm">Sign in to your Ethereal dashboard</p>
        </div>
        ...
        <div className="glass-card p-8">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div>
              ...
              <input type="email" className="glass-input" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} required autoComplete="email" />
            </div>
            <div>
              <label className="block text-[11px] font-mono tracking-widest uppercase text-[var(--text-muted)] mb-2">Password</label>
              <input type="password" className="glass-input" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} required autoComplete="current-password" />
            </div>

            {error && (
              <p className="text-xs text-[#FF5A79] bg-[#FF5A79]/10 border border-[#FF5A79]/20 rounded-md p-3">{error}</p>
            )}
        ...
        <p className="text-center mt-8 text-sm text-[var(--text-muted)]">
          No account yet?{' '}
          <Link href="/signup" className="text-[var(--green-start)] no-underline font-medium hover:text-[var(--green-end)] transition-colors">Create one</Link>
        </p>
      </div>
